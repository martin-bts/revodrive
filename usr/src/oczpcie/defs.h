...
#ifndef _OCZPCIE_DEFS_H_
#define _OCZPCIE_DEFS_H_

#define SAS_STATUS_BUF_SIZE 96
#define ATA_RESP_FIS_SIZE 24

struct oczpcie_dev_to_host_fis {
	u8     fis_type;	  /* 0x34 */
	u8     flags;
	u8     status;
	u8     error;
...
struct oczpcie_host_to_dev_fis {
        u8     fis_type;          /* 0x27 */
        u8     flags;
        u8     command;
        u8     features;
...
enum exec_status {
        OCZPCIE_SAM_STAT_CHECK_CONDITION = 2,

        OCZPCIE_NO_RESPONSE = 0x80,
        OCZPCIE_DATA_UNDERRUN,
        OCZPCIE_DATA_OVERRUN,
        OCZPCIE_NTERRUPTED,
        OCZPCIE_QUEUE_FULL,
        OCZPCIE_DEVICE_UNKNOWN,
        OCZPCIE_SG_ERR,
        OCZPCIE_ABORTED_TASK,
        OCZPCIE_OPEN_REJECT,
        OCZPCIE_PROTO_RESPONSE
};
...
struct oczpcie_task {
	...
	struct oczpcie_ata_task ata_task;
	...
	void   (*task_done)(struct oczpcie_task *);

	struct oczpcie_info *oczi;
	struct oczpcie_prv_info *mpi;
	struct oczpcie_slot_info *slot;
	struct bio *bio;	// the originating bio request
	struct dma_info dma_info;	// for non-bio requests
	int device_number;
	int *error_status;	// pointer to status to update for issue_command
	u64 lba;	// used for diagnostics
	u64 timeout;	// command timeout, means use default
	u32 phy_id;
};

#define	SLOT_WIDTH	11
#define	FIS_OFFS	0x800
#define	FIS_COUNT	17
#define	N_HOST	2
#define	N_PHY	4
#define	MAX_NCQ_DEPTH	32	// cannot be more than 63 without changing other structures
#define	MAX_NCQ_DEPTH_SHIFT	5	// log2 of above
#define	NCQ_MASK	0x1F	// mask for above
...
// how often the timeout check timer runs
#define	TIMER_TICK	(15 * HZ)

#define SAS_ADDR_SIZE        8
#define	OCZPCIE_SAM_STAT_GOOD	0


/* driver compile-time configuration */
enum driver_configuration {
	OCZPCIE_TX_RING_SZ		= 1024,	/* TX ring size (12-bit) must be big enough to contain 8 lots of 32 (N_PHY * MAX_NCQ_DEPTH) */
	OCZPCIE_RX_RING_SZ		= 1024, /* RX ring size (12-bit) */
					/* software requires power-of-2
					   ring size */
	OCZPCIE_SLOT_BUF_SZ		= 8192, /* cmd tbl + IU + status + PRD */
	OCZPCIE_SSP_CMD_SZ		= 64,	/* SSP command table buffer size */
	OCZPCIE_ATA_CMD_SZ		= 96,	/* SATA command table buffer size */
	OCZPCIE_OAF_SZ		= 64,	/* Open address frame buffer size */
	OCZPCIE_QUEUE_SIZE		= 64,	/* Support Queue depth */
};

/* unchangeable hardware details */
enum hardware_details {
	OCZPCIE_MAX_PHYS		= 8,	/* max. possible phys */
	OCZPCIE_MAX_PORTS		= 8,	/* max. possible ports */
	OCZPCIE_SOC_PHYS		= 4,	/* soc phys */
	OCZPCIE_SOC_PORTS		= 4,	/* soc phys */
	OCZPCIE_MAX_DEVICES	= 8,	/* max supported device */
};
...
enum hw_register_bits {
	...
	/* OCZPCIE_INT_STAT, OCZPCIE_INT_MASK */
	...
	/* OCZPCIE_Px_SER_CTLSTAT (per-phy control) */
	...
	/* OCZPCIE_Px_INT_STAT, OCZPCIE_Px_INT_MASK (per-phy events) */
...
enum oczpcie_info_flags {
	MVF_FLAG_MSI		= (1U << 3),	/* Using MSI interrupts*/
	MVF_FLAG_THREADED_IRQ = (1U << 4)	/* Using threaded interrupts, depends on MSI */
};

enum error_control_flags {
	ERR_CTRL_STOP_PHY = 1,
	ERR_CTRL_STOP_CONTROLLER = 2,
	ERR_CTRL_STOP_DRIVER = 4
};

enum oczpcie_event_flags {
	PHY_PLUG_EVENT		= (3U),
